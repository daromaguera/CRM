...
import { confirmedValidator, passwordValidator, requiredValidator } from '@core/utils/validators' // Import the validators
...
const validateCurrentPassword = (password) => {
  const validationMessage = requiredValidator(password)
  currentPasswordError.value = validationMessage === true ? '' : validationMessage
}

const validateNewPassword = (password) => {
  const validationMessage = passwordValidator(password)
  newPasswordError.value = validationMessage === true ? '' : validationMessage
}

const validateConfirmPassword = (password) => {
  const validationMessage = confirmedValidator(password, newPassword.value)
  confirmPasswordError.value = validationMessage === true ? '' : validationMessage
}
  ...
  if (!currentPasswordError.value && !newPasswordError.value && !confirmPasswordError.value) {
    ...
    try {
      const response = await axios.put(
        `/api/user/update/password/${authUserStore.authUser.id}`,
        {
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard title="Change Password" class="rounded-xl">
        <VForm>
            ...
            <VRow>
              <VCol cols="12" md="6">
                ...
                <AppTextField
                  ...
                  autocomplete="on"
                  placeholder="············"
                  @blur="validateCurrentPassword(currentPassword)"
            ...
            <VRow>
              <VCol cols="12" md="6">
                ...
                <AppTextField
                  ...
                  @blur="validateNewPassword(newPassword)"
          ...
          <VCardText class="d-flex flex-wrap gap-4">
            <VBtn :loading="isLoading" @click="validateForm">Save changes</VBtn>
    ...
    <VCol cols="12">
      <VCard title="Two-steps verification" class="rounded-xl">
        <VCardText>
    ...
    <VCol cols="12" style="display: none">
      ...
      <VCard title="Create an API key" class="rounded-xl">
        <VRow no-gutters>
          <!-- 👉 Choose API Key -->
    ...
    <VCol cols="12">
      <!-- 👉 Table -->
      <VCard title="Recent Devices" class="rounded-xl">
        <VDivider />
